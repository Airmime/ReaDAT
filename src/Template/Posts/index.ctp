...
$this->start('queue'); ?>
    <p>
        <?= $this->Html->image("content/bloc/tools/queue/queue.png", ["alt" => "Queue"]); ?>
        All
        <?php if($allcounter != 0){ ?>
            <span id="counterread"><?= $allcounter; ?></span>
        <?php } ?>
    </p>
<?php $this->end();
...
<?php if($allcounter != 0){ ?>
    <button onclick="javascript:read('all',null,null,null)" id="buttonallread">I read everything</button>
<?php } ?>
<?php $this->end();
...
<div class="first">
    <div class="text">
        <p>
            Hi,<br/>
            Welcome on Readat. If you start by adding a website?
        </p>
    </div>
